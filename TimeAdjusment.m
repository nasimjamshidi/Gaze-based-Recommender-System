...
%     imwrite(RGB,[int2str(kk) '.png']);
    ...
    for i = 2:size(Fixed,1)
     ...
     Img = run_antonioGaussian(Img, 25);
                    ...
                    for ii = 2: size(Fixed,1)
                        if Fixed(ii,1)<1100 && Fixed(ii,2)<400

                            obj(kk,1) = obj(kk,1) + 1;

                        elseif Fixed(ii,1)<1100 && Fixed(ii,2)<600

                             obj(kk,2) = obj(kk,2) + 1;

                         elseif Fixed(ii,1)>1100 && Fixed(ii,2)<400
